...
if __name__ == '__main__':
    ...
    words = Counter()
    size_train_text = update_vocab("data/olid-train.csv", words)
    size_dev_text = update_vocab("data/olid-subset-diagnostic-tests.csv", words)
    size_test_text = update_vocab("data/olid-test.csv", words)
    print("- done.")

    # Build tag vocab with train and test datasets
    print("Building tag vocabulary...")
    tags = Counter()
    size_train_tags = update_vocab("data/olid-train.csv", tags)
    size_dev_tags = update_vocab("data/olid-subset-diagnostic-tests.csv", tags)
    size_test_tags = update_vocab("data/olid-test.csv", tags)
    print("- done.")

    # Assert same number of examples in datasets
    assert size_train_text == size_train_tags
    assert size_dev_text == size_dev_tags
    assert size_test_text == size_test_tags
    ...
    sizes = {
        'train_size': size_train_text,
        'dev_size': size_dev_text,
        'test_size': size_test_text,
